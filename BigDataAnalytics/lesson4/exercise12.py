...
df = pd.read_csv(PATH + CLEAN_DATA, skiprows=1,
                 sep=',', names=('Review', 'Rating'))

print(df.head(10))

# Prepare the data.
df['ReviewAdjusted'] = create_tokenized_array(df['Review'])
df['ReviewAdjusted'] = remove_stop_words(df['ReviewAdjusted'])
df['ReviewAdjusted'] = stem_words(df['ReviewAdjusted'])
vectorizedList = vectorize_list(df['ReviewAdjusted'])

# Get predictions and scoring data.
# Target is the rating that we want to predict.
X_test, y_test, y_predicted = model_and_predict(vectorizedList, df['Rating'])
...
def generate_word_list(word_df, score_start, score_end, n_gram_size):
    result_df = word_df[(word_df['Rating'] >= score_start) & \
                       (word_df['Rating'] <= score_end)]

    sentences = [sentence.split() for sentence in result_df['ReviewAdjusted']]
    word_array = []
    for i in range(0, len(sentences)):
...
MIN_SCORE = 1
SIZE = 3
counter_list = generate_word_list(dfSub, MIN_SCORE, MIN_SCORE, SIZE)

SIZE = 4
counter_list = generate_word_list(dfSub, MAX_SCORE, MAX_SCORE, SIZE)
