...
neuron_list = [5, 10, 20, 50, 100]

# Build model
def create_model(neurons=13):
    model = Sequential()
    model.add(Dense(neurons, input_dim=13, kernel_initializer='normal',
                    activation='relu'))
...
for neuron_count in neuron_list:
    model = create_model(neuron_count)
    history = model.fit(X_train, y_train, epochs=100,
                        batch_size=10, verbose=1,
                        validation_data=(X_val, y_val))
    rmse = evaluate_model(model, X_test, y_test)
    network_stats.append({"rmse": rmse, "epochs": 100, "batch": 10, "neurons": neuron_count})
