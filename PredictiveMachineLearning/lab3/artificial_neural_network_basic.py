...
def feed_forward(X_scaled):
    ...
    # Show hte predictions after each epoch
    print(layer_2)
    return layer_1, layer_2


# Update weights and bias from front to back.
def back_propagate(weights_0_1, bias_0_1, weights_1_2, bias_1_2):
    y_df = pd.DataFrame(data=y_train, columns=['admitted'])

    # Calculate prediction error.
    layer_2_error = layer_2 - np.array(y_df['admitted']).reshape(-1, 1)
    print("Layer 2 Error: ", layer_2_error)
...
for i in range(EPOCHS):
    ...
    # Feed data forward.
    layer_1, layer_2 = feed_forward(X_train_scaled)

    # Back propagate to update weights and bias.
    weights_0_1, bias_0_1, weights_1_2, bias_1_2 = \
        back_propagate(weights_0_1, bias_0_1, weights_1_2, bias_1_2)
