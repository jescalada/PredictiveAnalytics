...
FILE = 'drugSales.csv'
df = pd.read_csv(ROOT_PATH + DATASET_DIR + FILE)
samples = np.array(df[['value']])
samples = df.value.dropna()
...
# Plot the cumulative density function for samples and chi distribution.
plt.plot(x, scipy.stats.chi.cdf(x, df=1), label='chi')